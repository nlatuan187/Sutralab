...
const AuroraMesh = () => {
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // Sutralab-inspired colors (Deep Blue, Cyan, with hints of warmth)
          vec3 deepBlue = vec3(0.0, 0.05, 0.2);
          vec3 midBlue = vec3(0.0, 0.2, 0.5);
          vec3 cyan = vec3(0.0, 0.6, 0.8);
          vec3 brightCyan = vec3(0.2, 0.8, 1.0);
          vec3 accent = vec3(0.0, 0.4, 0.6); // Muted accent
          
          // Create flowing color transitions
          vec3 color = deepBlue;
          
          // Add mid blue flows
          float midFlow = smoothstep(0.3, 0.7, aurora + streaks * 0.3);
          color = mix(color, midBlue, midFlow);
          ...
          color = mix(color, accent, accentFlow * 0.5);
    ...
    const animate = () => {
      material.uniforms.time.value += 0.005; // Slower animation for background
